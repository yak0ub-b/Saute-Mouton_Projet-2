...
N_ANGLES_SOLVEUR     = 32   # Directions discrètes testées par saut
N_PUISSANCES_SOLVEUR = 5    # Niveaux de puissance (1/N … N/N × VMAX)
PRECISION_ETAT       = 15   # Résolution grille déduplication (px)
...
def _simule_saut_etat(depart, vx, vy, lst_blocs, objectif):
    ...
    La stabilité se détecte par position quasi-identique sur STABLE_DUREE frames
    (on n'utilise pas collision() : après push-out le mouton est exactement au bord,
    ce qui ferait renvoyer False alors qu'il est bien posé).

    Retourne (personnage_final, victoire_atteinte, hors_ecran).
    """
    perso = {'x': depart['x'], 'y': depart['y'], 'vx': vx, 'vy': vy, 'colle': False}
    stable_count = 0
    prev_x, prev_y = perso['x'], perso['y']

    for _ in range(MAX_FRAMES_SAUT):
        for _ in range(N_ETAPES):
            if not perso.get('colle', False):
                pas_physique(perso)
...
def _arrondi_etat(perso):
    """Retourne une clé de grille (ix, iy, colle) pour dédupliquer les positions visitées."""
    return (round(perso['x'] / PRECISION_ETAT),
            round(perso['y'] / PRECISION_ETAT),
            perso.get('colle', False))


def solveur_naif(personnage_depart, objectif, lst_blocs, callback=None):
    """
    Résout le niveau par DFS (recherche en profondeur) avec une pile explicite.

    Algorithme : on empile des couples (position, chemin). À chaque itération,
    on dépile, on ignore si déjà visité, sinon on teste tous les sauts possibles
    et on empile les nouvelles positions (LIFO → profondeur d'abord).
    La terminaison est garantie par l'ensemble visite (déduplication).
...
def solveur_bfs(personnage_depart, objectif, lst_blocs, callback=None):
    ...
    Algorithme : on utilise une file (deque) au lieu d'une pile. On retire en tête
    (FIFO → largeur d'abord). La première solution trouvée est garantie d'être
    optimale en nombre de sauts (exploration niveau par niveau : 1 saut, 2 sauts…).
            ...
            if ev is not None:
                if type_ev(ev) == 'Quitte':
                    ...
                    def affiche_prog(pos):
                        ...
                        dessine_positions_explorees(pos)
                        mise_a_jour()
